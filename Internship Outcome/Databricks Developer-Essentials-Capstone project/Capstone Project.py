...
display(dbutils.fs.ls("dbfs:/dbacademy/harshaggarwal428@gmail.com/developer-essentials-capstone/lookup_data"))
...
def bronzeToSilver(bronzePath, silverPath, streamName, lookupDF):
  (spark.readStream
    .format("delta")
    .load(bronzePath)
    .select("eventParams.device_id","eventName","eventParams.client_event_time","eventDate")
    .join(lookupDF,"device_id")
...
spark.read.format("delta").load(outputPathSilver).display()
...
def silverToGold(silverPath, goldPath, queryName):
  (spark.readStream.format("delta")
   .load(silverPath)
   .withColumn("week",weekofyear(col("client_event_time")))
   .groupby("week").agg(approx_count_distinct("device_id",rsd=.01).alias("WAU"))
