...
streamorder_df=df.selectExpr("submittedAt as submitted_at","orderId as order_id","customerId as customer_id","salesRepId as sales_rep_id","shippingAddress.attention as shipping_address_attention","shippingAddress.address as shipping_address_address","shippingAddress.city as shipping_address_city","shippingAddress.state as shipping_address_state","shippingAddress.zip as shipping_address_zip")
streamorder_df.display()

# COMMAND ----------

x=streamorder_df.schema

# COMMAND ----------

stream_o_df=spark.readStream.schema(x1).option("maxFilesPerTrigger",1).json(stream_path)


# COMMAND ----------

stream_o_df=stream_o_df.selectExpr("submittedAt as submitted_at","orderId as order_id","customerId as customer_id","salesRepId as sales_rep_id","shippingAddress.attention as shipping_address_attention","shippingAddress.address as shipping_address_address","shippingAddress.city as shipping_address_city","shippingAddress.state as shipping_address_state","shippingAddress.zip as shipping_address_zip")
stream_o_df.display()

# COMMAND ----------

stream_o_df=(stream_o_df.withColumn("ingest_file_name",input_file_name())
             .withColumn("ingested_at",current_timestamp().cast("timestamp"))
             .withColumn("shipping_address_zip",col("shipping_address_zip").cast("integer"))
           .withColumn("submitted_at",to_timestamp("submitted_at").cast("timestamp"))
...
(stream_o_df.writeStream.format("delta").outputMode('append').partitionBy("submitted_yyyy_mm").trigger(processingTime="1 second")
                                       .queryName(orders_table).option("checkpointLocation",orders_checkpoint_path).table(orders_table))
...
linestream_df=spark.readStream.option("maxFilesPerTrigger",1).schema(x1).json(stream_path)
linestream_df.display()

# COMMAND ----------

linestream_df=linestream_df.withColumn("new_product",explode("products"))
linestream_df.display()


# COMMAND ----------

linestream_df=linestream_df.selectExpr("orderId as order_id","new_product.productId as product_id","new_product.quantity as product_quantity","new_product.soldPrice as product_sold_price")
...
linestream_df=(linestream_df.withColumn("ingest_file_name",input_file_name())
             .withColumn("ingested_at",current_timestamp().cast("timestamp"))
...
(linestream_df.writeStream.outputMode("append").format("delta").trigger(processingTime="1 second")
                                       .queryName(line_items_table).option("checkpointLocation",line_items_checkpoint_path).table(line_items_table))
