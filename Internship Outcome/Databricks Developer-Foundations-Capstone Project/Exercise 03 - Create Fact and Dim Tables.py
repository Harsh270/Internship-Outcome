...
display(dbutils.fs.ls(batch_source_path))
...
df=(df.withColumn("_error_ssn_format",(when(col("sales_rep_ssn").like("%-%"),lit(True)).otherwise(lit(False))).cast("boolean"))
  .withColumn("sales_rep_ssn",regexp_replace("sales_rep_ssn","-","").cast("long"))
...
df_sale=spark.sql("select * from batched_orders")
df_sale.display()

# COMMAND ----------

df_sale=(df_sale.withColumn("submitted_at",from_unixtime("submitted_at").cast("timestamp"))
  .withColumn("shipping_address_zip",col("shipping_address_zip").cast("integer")))

# COMMAND ----------

df_sale=df_sale.drop("sales_rep_ssn", "sales_rep_first_name", "sales_rep_last_name", "sales_rep_address", "sales_rep_city", "sales_rep_state", "sales_rep_zip","product_id", "product_quantity", "product_sold_price")
df_sale.display()

# COMMAND ----------

df_sale=df_sale.dropDuplicates(["submitted_at","order_id","customer_id","sales_rep_id","shipping_address_attention","shipping_address_address","shipping_address_city","shipping_address_state","shipping_address_zip"])
...
df_sale=df_sale.withColumn("submitted_yyyy_mm",date_format(col('submitted_at'),"yyyy-MM"))
...
df_sale.groupby("submitted_yyyy_mm").count().display()
...
df_sale.write.mode("overwrite").format("delta").partitionBy("submitted_yyyy_mm").saveAsTable(orders_table)
...
df_line=spark.table("batched_orders").select("order_id","product_id","product_quantity","product_sold_price","ingest_file_name" ,"ingested_at")
df_line.display()
...
df_line=(df_line.withColumn("product_quantity",col("product_quantity").cast("integer"))
       .withColumn("product_sold_price",col("product_sold_price").cast(DecimalType(10,2))))

# COMMAND ----------

df_line.write.mode("overwrite").format("delta").saveAsTable(line_items_table)
