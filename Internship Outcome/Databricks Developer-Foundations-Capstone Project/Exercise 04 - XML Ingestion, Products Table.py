...
df=spark.read.option("inferSchema","True").option("rootTag","products").option("rowTag","product").format("xml").load(products_xml_path)
df.display()
...
df=df.withColumnRenamed("_product_id","product_id")
...
df=df.selectExpr("product_id","color","model_name","model_number","_base_price as base_price","_color_adj as color_adj","_size_adj as size_adj","usd as price","size")
...
df.write.mode("overwrite").format("delta").saveAsTable(products_table)
